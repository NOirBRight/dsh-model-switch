	...
	factory: (require) => {
		...
		*/
		function decodeCapabilitiesSnapshot(value) {
			const payload = record$2(value);
			const revision = payload?.revision;
			if (typeof revision !== "number" || !Number.isSafeInteger(revision) || revision < 0) return void 0;
			const capabilities = payload?.capabilities === void 0 ? void 0 : record$2(payload.capabilities);
			if (capabilities === void 0) return void 0;
			const search = record$2(capabilities.searchProviderAdapters);
		...
		*/
		...
		//#endregion
		//#region node_modules/.pnpm/dsh-llm-providers-ui@file+..+dsh-providers-settings-a_462103270184ff59992e9c400360faf3/node_modules/dsh-llm-providers-ui/lib/order.js
		...
		* Kept as literals: the Antigravity plugin is not a Model Switch dependency.
		*/
		const ANTIGRAVITY_BINDING_CHANNEL = "/dsh-acp-antigravity";
		const ANTIGRAVITY_BINDING_ENDPOINT = "activity/binding";
		...
		*/
		function decodeBindingProvider(value) {
			const reply = record(value);
			if (reply === void 0) return void 0;
			if (reply.provider === null) return null;
			return reply.provider === "antigravity" ? ANTIGRAVITY_PROVIDER_KEY : void 0;
		}
		/**
		* Read one session native binding; undefined when the plugin is absent,
		* unreachable, or malformed. Never throws.
		*/
		async function fetchSessionBinding(rpc, sessionId) {
			if (rpc === void 0) return void 0;
			try {
				const result = await rpc.call(ANTIGRAVITY_BINDING_CHANNEL, ANTIGRAVITY_BINDING_ENDPOINT, { sessionId }, void 0);
				return result.ok ? decodeBindingProvider(result.value) : void 0;
			} catch {
				return;
			}
		}
		...
		*/
		function isProviderAllowed(state, provider, currentProvider) {
			if (!state.failed) return providerSelectable(state.provider, provider);
			if (state.provider === "antigravity" || currentProvider === "antigravity") return provider === ANTIGRAVITY_PROVIDER_KEY;
			return true;
		}
		/**
		...
		*/
		function effectiveProviderLock(state, currentProvider) {
			if (!state.failed) return state.provider;
			if (state.provider === "antigravity" || currentProvider === "antigravity") return ANTIGRAVITY_PROVIDER_KEY;
			return null;
		}
		/**
		...
		*/
		...
		function ComposerPicker({ locked, providerLock = null, available, directory, t, draft, onDraftChange, embedded, tone, resolveInteractionOperations, roleOf }) {
		...
		function PlanReviewState({ matched, review, available, providerLock, lockFailed, directory, t, resolveInteractionOperations, roleOf }) {
			...
			ctx.inject([
				...
				const directoryFace = (sessionId) => {
					...
					const antigravityPresent = () => {
						const declarations = scope.get("providerDirectory", false);
						return typeof declarations?.reader === "function" && declarations.reader("antigravity") !== void 0;
					};
					const readLock = async (previous) => {
						if (!antigravityPresent()) return {
							provider: null,
							failed: false
						};
					...
					const roleOf = (key) => readProviderRole(scope.get("providerDirectory", false), key);
