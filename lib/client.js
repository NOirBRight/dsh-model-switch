	...
	factory: (require) => {
		...
		*/
		...
		const zh$1 = {
			...
			defaultBadge: "默认",
			cancel: "取消",
			compactOnSwitch: "发送时自动检查并压缩上下文",
			compactOnSwitchHelp: "发送消息采用新模型时，先检查上下文容量；必要时使用切换前的模型压缩历史，再继续请求。仅选择模型不会触发压缩。压缩会产生额外模型用量，摘要可能省略细节。",
			main: "主模型",
			provider: "提供商",
			model: "模型",
		...
		const en$1 = {
			...
			cancel: "Cancel",
			compactOnSwitch: "Check and compact context on send",
			compactOnSwitchHelp: "When a sent message uses a new model, check context capacity first. If needed, compact history with the previous model, then continue. Choosing a model does not compact. Compaction uses extra model quota and may omit details.",
			main: "Main model",
			provider: "Provider",
			model: "Model",
		...
		async function fetchSessionBinding(rpc, sessionId, sources) {
			...
			const replies = await Promise.all(sources.map(async (source) => {
				try {
					const result = await rpc.call(source.channel, source.endpoint, { sessionId }, void 0);
		...
		*/
		...
		//#endregion
		//#region ../../compat-dsh-peer-star/dsh-model-switch/node_modules/.pnpm/dsh-llm-providers-ui@https+++github.com+NOirBRight+dsh-llm-providers-ui+releases+downlo_f61686d3b6c82ce50cddd721feeae8d3/node_modules/dsh-llm-providers-ui/lib/order.js
		...
		function apply(ctx) {
			...
			ctx.slots.inject("settings.section", () => ctx.slots.register({
				...
				inject: () => ({
					...
					setCompactOnSwitch: (value) => owned.set("compactOnSwitch", value),
					providerRoleOf,
					loadCatalog: async () => {
