window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		//#endregion
		//#region node_modules/.pnpm/dsh-llm-providers-ui@https+++github.com+NOirBRight+dsh-llm-providers-ui+releases+downlo_a329889273b72a8a8989d3db937845a3/node_modules/dsh-llm-providers-ui/lib/order.js
		...
		*/
		function effectiveProviderLock(state, currentProvider, active = false) {
			if (active && currentProvider === "antigravity") return ANTIGRAVITY_PROVIDER_KEY;
		...
		*/
		...
		function ComposerPicker({ locked, providerLock = null, agentLocked = false, available, directory, t, draft, onDraftChange, embedded, tone, resolveInteractionOperations, roleOf }) {
			...
			const choiceAllowed = (provider) => providerSelectable(providerLock, provider) && !(agentLocked && isAgentRole(roleOf?.(provider)));
			const reload = () => {
				if (lockedRef.current) return;
		...
		function PlanReviewState({ matched, review, available, providerLock, agentLocked, lockFailed, directory, t, resolveInteractionOperations, roleOf }) {
			...
			const executionAllowed = execution !== void 0 && providerSelectable(providerLock, execution.provider) && !(agentLocked && isAgentRole(roleOf?.(execution.provider)));
