	...
	factory: (require) => {
		...
		*/
		...
		function ComposerPicker({ locked, available, directory, t, draft, onDraftChange, embedded, tone, externalTargets = [], externalTargetsLabel, externalSelection, onExternalTargetChange, resolveInteractionOperations }) {
			...
			const currentCanBeUsed = draft !== void 0 || state.routable !== false;
			...
			const applySelection = (next) => {
				...
				if (currentCanBeUsed && state.current?.provider === next.provider && state.current.model === next.model && state.current.reasoningEffort === next.reasoningEffort) {
					returnToRoot();
					return;
				}
			...
			const triggerAria = selectedExternal !== void 0 ? selectedExternal.label : currentSelection === null ? t("trigger.selectAria") : t("trigger.aria", { model: triggerLabel });
		...
		async function respondAnswer(wait, id, label, rejectedMessage, terminalRejection = false) {
			const target = wait;
			if (typeof target.answer === "function") try {
				await target.answer({ answers: [{
					id,
					selected: [label]
				}] });
				return;
		...
		function questionsOfWait(wait) {
			const raw = wait;
			if (Array.isArray(raw.questions)) return raw.questions;
			if (raw.payload !== void 0 && typeof raw.payload === "object" && raw.payload !== null && Array.isArray(raw.payload.questions)) return raw.payload.questions;
			return [];
		}
		function PlanReviewCard(props) {
			const snapshot = props.useDirectory((value) => value);
			const order = props.useProviderOrder((value) => value);
			const review = planReviewOf(questionsOfWait(props.matched));
