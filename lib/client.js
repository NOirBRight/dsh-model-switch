window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			search: "Web 搜索",
			vision: "视觉理解",
			visionHelp: "read_image 会把本地路径或公网图片交给所选 LLM 独立分析，并只把文本描述返回当前会话。任何已注册、声明接受图片输入的模型都可以选用；主模型可以没有视觉能力；普通聊天附件不会被预处理。不接受图片的模型会在调用时明确失败。",
			image: "图像生成",
			unavailable: "未接入",
			loading: "正在加载设置…",
			readonly: "设置为只读",
		...
		const en = {
			...
			search: "Web search",
			vision: "Vision",
			visionHelp: "read_image sends a local path or public URL to the selected LLM for independent analysis and returns only the text description. Any registered model that declares image input can be chosen. Main can be text-only; ordinary chat attachments are not preprocessed. A selected model that does not accept image input fails the call instead of falling back.",
		...
		function apply(ctx) {
			...
			const subagent = deriveSettingsScope(ctx.settingsScope.bind({
				namespace: MODEL_SWITCH_SETTINGS_ID,
				decode: decodeModelSwitchSettings
			}), deriveSubagentSettings, SUBAGENT_SETTINGS_FIELDS);
