window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#endregion
		//#region node_modules/.pnpm/dsh-llm-providers-ui@file+..+dsh-llm-providers-ui+fixtures+alpha4+tarballs+dsh-llm-prov_67473584235a93930095c68fa9b64c25/node_modules/dsh-llm-providers-ui/lib/order.js
		...
		*/
		...
		function ComposerPicker({ locked, available, directory, t, draft, onDraftChange, embedded, tone, resolveInteractionOperations }) {
		...
		function overlayPickerSnapshot(base, extra, current) {
		...
		function createExternalCatalogStore(base) {
			let extra = [];
			let current = null;
			let lastBase;
			let lastExtra = extra;
			...
			return {
				store: {
					...
					getSnapshot: () => {
						...
						if (lastSnapshot !== void 0 && lastBase === next && lastExtra === extra && lastCurrent === current) return lastSnapshot;
						lastBase = next;
						lastExtra = extra;
						lastCurrent = current;
		...
		function externalCatalogLoader(ctx) {
			let value;
			try {
				value = ctx.get("connection", false);
			} catch {
				return;
			}
			if (value === null || typeof value !== "object" || !("rpc" in value)) return void 0;
			const rpc = value.rpc;
			return async () => {
				try {
					const result = await rpc.call("/dsh-acp-antigravity", "catalog", {}, void 0);
				...
				const directoryFace = (sessionId) => {
					...
					return {
						...
						load: () => {
							...
							if (loadExternalCatalog !== void 0) loadExternalCatalog().then(overlay.setExtra).catch(() => overlay.setExtra([]));
						},
