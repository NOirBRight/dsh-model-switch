		...
		*/
		...
		function ComposerPicker({ locked, available, directory: state, getDirectorySnapshot, load, select, t, draft, onDraftChange, embedded, tone, externalTargets = [], externalTargetsLabel, externalSelection, onExternalTargetChange, resolveInteractionOperations }) {
			...
			(0, react.useLayoutEffect)(() => {
				...
				const safeRight = `max(${Math.min(Math.max(gutter, window.innerWidth - rect.right), Math.max(gutter, window.innerWidth - gutter - preferredWidth))}px, calc(env(safe-area-inset-right) + ${gutter}px))`;
				setMenuStyle({
					position: "fixed",
					right: safeRight,
