...
try {
  ...
  const calls = parent.filter(event => event.type === 'assistant/message').flatMap(event => event.data.message.content).filter(block => block.type === 'tool-call')
  assert.deepEqual(calls.map(call => call.name), ['subagent'], 'Native child must be spawned with subagent, not delegate')
  assert.equal(JSON.parse(calls[0].arguments).run_in_background, false, 'This QA covers foreground one-shot spawning')
