...
// Verified 015 migration target: development pins may track it instead of the legacy 0.1.2-rc.1.
const TARGET_015_RC1 = '0.1.5-rc.1'
const CORDIS = '4.0.2'
...
function checkAlpha4Manifest(manifest, label) {
  ...
  for (const section of ['dependencies', 'optionalDependencies', 'peerDependencies', 'devDependencies']) {
    for (const [name, range] of Object.entries(manifest[section] ?? {})) {
      ...
      const verifiedDevPin = section === 'devDependencies' && (range === RC1 || range === TARGET_015_RC1)
      if (name.startsWith('@deepseek-ai/dsh-') && range !== ALPHA4 && !verifiedDevPin && !(satisfies(ALPHA4, range) && satisfies(RC1, range)) && !(capturedOfficialWorkspace && range === 'workspace:^')) fail(label + ' has a DSH range that excludes Alpha.4, 0.1.2-rc.1, or 0.1.5-rc.1: ' + name + ' ' + range)
